-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Autonomous Multi-Agent AI Coaching System (SAFE VERSION)
-- Date: 2026-03-16
--
-- Bugs fixed vs original:
--   #1 CRITICAL — weight trend CASE: swapped order so losing_fast/gaining_fast
--                 are evaluated before losing/gaining (CASE short-circuits)
--   #2 CRITICAL — run_decision_engine: replaced broken EXECUTE format() with
--                 safe named-column evaluation via explicit IF/ELSIF chain;
--                 no dynamic SQL injection possible
--   #3 HIGH     — dispatch_ai_actions: wrapped jsonb_array_elements_text() with
--                 COALESCE(..., '[]'::jsonb) to prevent NULL crash
--   #4 HIGH     — pg_cron: check pg_extension (enabled) not pg_available_extensions
--                 (installable); use ON CONFLICT UPDATE for true idempotency
--   #5 MEDIUM   — compute_user_state: removed unused v_events_14d variable/query;
--                 incorporated 14d events into engagement_score instead
--   #6 MEDIUM   — log_user_event trigger: fixed ON CONFLICT SET to use unqualified
--                 table name (not public.ai_user_state.col which is invalid syntax)
--   #7 LOW      — ai_agents INSERT: explicit column list guards against schema drift
--
-- Strategy:
...
-- ── STEP 1: Extend existing ai_agents ────────────────────────────────────
...
-- Seed five core agents — idempotent via ON CONFLICT
-- FIX #7: explicit column list so INSERT never references columns that might not exist
INSERT INTO public.ai_agents
  (agent_name, description, model, temperature, max_tokens, active, agent_type, priority)
VALUES
  ('nutrition_agent',
   'Analyses food logs, macro adherence, and caloric balance. Generates meal optimisation recommendations.',
   'cloud_model', 0.3, 1500, true, 'analysis', 10),
  ('training_agent',
   'Reviews workout history, volume, intensity, and progression. Adjusts training plans adaptively.',
   'cloud_model', 0.3, 1500, true, 'analysis', 20),
  ('recovery_agent',
   'Monitors sleep quality, fatigue signals, and recovery metrics. Issues rest and de-load recommendations.',
   'cloud_model', 0.2, 1000, true, 'analysis', 5),
  ('behavior_agent',
   'Tracks habit adherence, engagement patterns, and motivational signals. Drives behavioural nudges.',
   'cloud_model', 0.4, 1200, true, 'analysis', 30),
  ('progress_agent',
   'Evaluates goal progression, weight trends, body composition changes, and milestone proximity.',
   'cloud_model', 0.3, 1200, true, 'analysis', 25)
...
-- Guard: if the table already existed from a partial prior run, add missing columns
...
-- ── STEP 4: ai_user_state — Computed Feature Vector ──────────────────────
...
-- Guard: add any columns missing from a partial prior run
ALTER TABLE public.ai_user_state
  ADD COLUMN IF NOT EXISTS user_id             uuid        REFERENCES public.profiles(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS engagement_score    numeric     DEFAULT 0,
  ADD COLUMN IF NOT EXISTS fatigue_score       numeric     DEFAULT 0,
  ADD COLUMN IF NOT EXISTS adherence_score     numeric     DEFAULT 0,
  ADD COLUMN IF NOT EXISTS momentum_score      numeric     DEFAULT 0,
  ADD COLUMN IF NOT EXISTS readiness_score     numeric     DEFAULT 0,
  ADD COLUMN IF NOT EXISTS weight_trend        text        DEFAULT 'stable',
...
-- Ensure UNIQUE constraint exists on user_id (skipped if table pre-existed)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.ai_user_state'::regclass
      AND contype = 'u'
      AND conname LIKE '%user_id%'
  ) THEN
    ALTER TABLE public.ai_user_state ADD CONSTRAINT ai_user_state_user_id_key UNIQUE (user_id);
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.ai_decision_rules (
  ...
  description     text,
  condition_field text    NOT NULL DEFAULT 'engagement_score',
  condition_op    text    NOT NULL DEFAULT 'gt'
    CHECK (condition_op IN ('gt','lt','gte','lte','eq')),
  condition_value numeric NOT NULL DEFAULT 0,
  action_type     text    NOT NULL DEFAULT 'generate_insight',
...
-- Guard: if table existed from prior run with old condition_sql schema, add new columns
ALTER TABLE public.ai_decision_rules
  ADD COLUMN IF NOT EXISTS condition_field text    DEFAULT 'engagement_score',
  ADD COLUMN IF NOT EXISTS condition_op    text    DEFAULT 'gt',
  ADD COLUMN IF NOT EXISTS condition_value numeric DEFAULT 0,
  ADD COLUMN IF NOT EXISTS action_type     text    DEFAULT 'generate_insight',
  ADD COLUMN IF NOT EXISTS action_payload  jsonb   DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS active          boolean DEFAULT true,
  ADD COLUMN IF NOT EXISTS locale_filter   text[],
  ADD COLUMN IF NOT EXISTS updated_at      timestamptz DEFAULT now();

-- Ensure UNIQUE constraint exists on rule_name
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.ai_decision_rules'::regclass
      AND contype = 'u'
      AND conname LIKE '%rule_name%'
  ) THEN
    ALTER TABLE public.ai_decision_rules ADD CONSTRAINT ai_decision_rules_rule_name_key UNIQUE (rule_name);
  END IF;
END $$;
...
INSERT INTO public.ai_decision_rules
  (rule_name, description, condition_field, condition_op, condition_value, action_type, action_payload, priority)
VALUES
  ('high_fatigue_alert',
...
-- Guard: add columns missing from partial prior run
...
ALTER TABLE public.ai_action_outcomes
  ...
  ADD COLUMN IF NOT EXISTS user_responded boolean DEFAULT false,
...
ALTER TABLE public.ai_agent_outputs
  ...
  ADD COLUMN IF NOT EXISTS resolved_by_coordinator    boolean     DEFAULT false,
  ADD COLUMN IF NOT EXISTS locale                     text        DEFAULT 'en',
  ADD COLUMN IF NOT EXISTS created_at                 timestamptz DEFAULT now();
...
ALTER TABLE public.ai_coaching_summaries
  ...
  ADD COLUMN IF NOT EXISTS recommendation_ids   uuid[]      DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS plan_ids             uuid[]      DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS overall_confidence   numeric     DEFAULT 0.8,
  ADD COLUMN IF NOT EXISTS user_state_snapshot  jsonb       DEFAULT '{}'::jsonb,
...
ALTER TABLE public.ai_cohort_metrics
  ...
  ADD COLUMN IF NOT EXISTS avg_weight_change numeric DEFAULT 0,
...
-- Ensure UNIQUE constraint exists on (cohort_key, metric_date)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.ai_cohort_metrics'::regclass
      AND contype = 'u'
      AND conname LIKE '%cohort%'
  ) THEN
    ALTER TABLE public.ai_cohort_metrics ADD CONSTRAINT ai_cohort_metrics_key_date_key UNIQUE (cohort_key, metric_date);
  END IF;
END $$;
...
-- FIX #6: ON CONFLICT SET clause uses unqualified column name (not
--          `public.ai_user_state.next_action_due_at` which is invalid syntax)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- FIX #6: correct ON CONFLICT syntax — unqualified column reference, not schema-qualified
  ...
  ON CONFLICT (user_id) DO UPDATE
    SET next_action_due_at = LEAST(
      ai_user_state.next_action_due_at,   -- unqualified: refers to existing row
...
-- Attach event triggers (DROP IF EXISTS first for idempotency)
...
-- FIX #1: weight trend CASE order corrected (check ±2 before ±1)
-- FIX #5: removed unused v_events_14d variable; use total 14d events in
--         engagement_score instead to give it value
-- ═══════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_weight_trend      text     := 'stable';
  v_events_7d         integer  := 0;
  v_events_14d        integer  := 0;   -- FIX #5: declared and used
...
BEGIN
  ...
  -- Food logs: distinct days and averages
  SELECT
  ...
  -- FIX #1: check larger delta first so losing_fast / gaining_fast are reachable
  IF v_latest_weight_kg IS NOT NULL AND v_prev_weight_kg IS NOT NULL THEN
    v_weight_delta := v_latest_weight_kg - v_prev_weight_kg;
    IF    v_weight_delta <= -2.0 THEN v_weight_trend := 'losing_fast';
    ELSIF v_weight_delta <= -0.5 THEN v_weight_trend := 'losing';
    ELSIF v_weight_delta >=  2.0 THEN v_weight_trend := 'gaining_fast';
    ELSIF v_weight_delta >=  0.5 THEN v_weight_trend := 'gaining';
    ELSE                               v_weight_trend := 'stable';
    END IF;
  END IF;

  -- Event counts (7d and 14d)
  SELECT COALESCE(count(*), 0) INTO v_events_7d
  FROM public.user_events
  WHERE user_id = p_user_id AND created_at >= now() - interval '7 days';

  -- FIX #5: actually use v_events_14d (recency factor: events in second week show base activity)
  ...
  -- Engagement: recent 7d events weighted more than second-week events
  -- FIX #5: v_events_14d (second-week events) contributes a recency-discounted bonus
  v_engagement_score := LEAST(100, GREATEST(0,
    (v_events_7d        * 3.0) +
    (v_events_14d       * 1.0) +   -- second-week events at 33% weight
    (v_workouts_7d      * 10.0) +
    (v_food_days_7d     * 8.0) +
  ...
  -- Readiness: sleep quality minus fatigue drag
  ...
  -- Intervention check
  IF    v_fatigue_score > 80                          THEN v_intervention := true; v_intervention_reason := 'high_fatigue';
  ELSIF v_engagement_score < 20                       THEN v_intervention := true; v_intervention_reason := 'low_engagement';
  ELSIF v_food_days_7d = 0 AND v_workouts_7d = 0     THEN v_intervention := true; v_intervention_reason := 'inactive_user';
  END IF;

  -- Upsert ai_user_state
  INSERT INTO public.ai_user_state (
    user_id,
    engagement_score, fatigue_score, adherence_score, momentum_score, readiness_score,
    ...
    p_user_id,
    round(v_engagement_score, 1), round(v_fatigue_score, 1),
    round(v_adherence_score, 1),  round(v_momentum_score, 1),
    round(v_readiness_score, 1),
    v_weight_trend, v_calorie_trend, v_training_trend,
    jsonb_build_object(
    ...
    v_intervention, v_intervention_reason,
  ...
  -- Sync backward-compat ai_coaching_state
...
-- ═══════════════════════════════════════════════════════════════════════════
-- DECISION ENGINE
-- FIX #2: replaced broken EXECUTE format('SELECT ($1.%s)', condition_sql)
--         with a safe IF/ELSIF chain that reads named columns by value.
--         No dynamic SQL — no injection, no syntax errors.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    -- Evaluate each active rule using safe named-column dispatch
    -- FIX #2: no dynamic SQL — explicit CASE on condition_field column name
    FOR v_rule IN
      SELECT *
    ...
    LOOP
      -- Extract the numeric value for the field named in condition_field
      v_field_value := CASE v_rule.condition_field
      ...
      -- Skip rules that reference unknown fields
      CONTINUE WHEN v_field_value IS NULL;

      -- Apply operator safely — no dynamic SQL
      v_matches := CASE v_rule.condition_op
        WHEN 'gt'  THEN v_field_value >  v_rule.condition_value
        WHEN 'lt'  THEN v_field_value <  v_rule.condition_value
        WHEN 'gte' THEN v_field_value >= v_rule.condition_value
        WHEN 'lte' THEN v_field_value <= v_rule.condition_value
        WHEN 'eq'  THEN v_field_value =  v_rule.condition_value
        ELSE false
      END;

      -- Enqueue action — deduplicated within 6-hour window
      IF v_matches THEN
        ...
        SELECT
          ...
          'rule_engine',
          v_rule.id,
          v_rule.action_payload || jsonb_build_object(
            'matched_field', v_rule.condition_field,
            'matched_value', v_field_value,
            'threshold',     v_rule.condition_value
          ),
          v_rule.priority,
          v_user.locale,
          'rule',
    ...
    -- Scheduled general analysis if no rule-triggered action in last 6h
    IF NOT EXISTS (
      SELECT 1 FROM public.ai_actions
      WHERE user_id     = v_user.user_id
        AND status      = 'pending'
      ...
      INSERT INTO public.ai_actions (
        ...
        v_user.locale,
        CASE
          WHEN v_user.momentum_score > 60 THEN 'small_model'  -- healthy user: save cost
          ELSE 'cloud'                                          -- struggling user: full LLM
        END
      );
    END IF;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ACTION DISPATCHER
-- FIX #3: COALESCE on payload->'agents' prevents NULL crash in
  ...
  LOOP
    ...
    IF v_action.action_type = 'run_agent_analysis' THEN
      FOR v_agent IN
        ...
        WHERE active = true
          AND (
            -- No specific agent requested → dispatch to all
            (v_action.agent_name IS NULL AND NOT jsonb_typeof(COALESCE(v_action.payload->'agents', 'null'::jsonb)) = 'array')
            OR agent_name = v_action.agent_name
            -- FIX #3: COALESCE to empty array prevents NULL crash
            OR agent_name = ANY(
                ARRAY(
      ...
      END LOOP;
    ELSE
      -- Single task for non-analysis actions
      INSERT INTO public.ai_agent_tasks (
        user_id, agent_id, task_type, input_data, status, priority,
        action_id, locale, reasoning_layer
      )
...
-- ═══════════════════════════════════════════════════════════════════════════
-- COORDINATOR — unchanged (was already safe)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FOR v_outputs IN
    ...
    WHERE user_id              = p_user_id
      AND coordinator_processed = false
      AND created_at            > now() - interval '24 hours'
    ORDER BY
      array_position(v_agent_order, agent_name),
      confidence DESC
  LOOP
    ...
    IF COALESCE(v_outputs.content->>'en', '') != '' THEN
      v_merged_en := v_merged_en || E'\n\n' || v_outputs.agent_name || ': ' || (v_outputs.content->>'en');
    END IF;
    IF COALESCE(v_outputs.content->>'fr', '') != '' THEN
      v_merged_fr := v_merged_fr || E'\n\n' || v_outputs.agent_name || ': ' || (v_outputs.content->>'fr');
    END IF;
    IF COALESCE(v_outputs.content->>'ar', '') != '' THEN
      v_merged_ar := v_merged_ar || E'\n\n' || v_outputs.agent_name || ': ' || (v_outputs.content->>'ar');
    END IF;

    v_output_ids := v_output_ids || ARRAY[v_outputs.id];
    IF v_outputs.related_insight_id        IS NOT NULL THEN v_insight_ids := v_insight_ids || ARRAY[v_outputs.related_insight_id]; END IF;
    IF v_outputs.related_recommendation_id IS NOT NULL THEN v_rec_ids    := v_rec_ids    || ARRAY[v_outputs.related_recommendation_id]; END IF;
    IF v_outputs.related_plan_id           IS NOT NULL THEN v_plan_ids   := v_plan_ids   || ARRAY[v_outputs.related_plan_id]; END IF;

    v_confidence  := v_confidence + v_outputs.confidence;
    v_conf_count  := v_conf_count + 1;

    UPDATE public.ai_agent_outputs
    SET coordinator_processed = true,
        coordinator_priority  = array_position(v_agent_order, v_outputs.agent_name)
  ...
  INSERT INTO public.ai_coaching_summaries (
    ...
    overall_confidence, user_state_snapshot, locale
  ) VALUES (
    p_user_id,
    ...
    v_primary_agent, v_conflicts,
    v_insight_ids, v_rec_ids, v_plan_ids,
...
BEGIN
  INSERT INTO public.ai_cohort_metrics (
    cohort_key, metric_date, user_count, avg_engagement,
    avg_fatigue, avg_adherence, avg_momentum, active_users
  )
  SELECT
    ...
    count(*) FILTER (WHERE engagement_score > 40)
  FROM public.ai_user_state
...
CREATE POLICY "user_events_own"           ON public.user_events           FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_user_state_own"         ON public.ai_user_state         FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_decision_rules_read"    ON public.ai_decision_rules     FOR SELECT USING (auth.role() IN ('authenticated','service_role'));
CREATE POLICY "ai_actions_own"            ON public.ai_actions            FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_action_outcomes_own"    ON public.ai_action_outcomes    FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_agent_outputs_own"      ON public.ai_agent_outputs      FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_coaching_summaries_own" ON public.ai_coaching_summaries FOR ALL    USING (user_id = auth.uid() OR auth.role() = 'service_role');
CREATE POLICY "ai_cohort_metrics_read"    ON public.ai_cohort_metrics     FOR SELECT USING (auth.role() IN ('authenticated','service_role'));
CREATE POLICY "ai_worker_logs_service"    ON public.ai_worker_logs        FOR ALL    USING (auth.role() = 'service_role');

-- ═══════════════════════════════════════════════════════════════════════════
-- SCHEDULER
-- FIX #4: check pg_extension (enabled extensions) NOT pg_available_extensions
--         (installable but not yet enabled). Use cron.unschedule() before
--         re-scheduling so repeated runs don't create duplicate jobs.
-- ═══════════════════════════════════════════════════════════════════════════

DO $$
BEGIN
  -- FIX #4: pg_extension = actually enabled; pg_available_extensions = just installable
  IF EXISTS (
    SELECT 1 FROM pg_extension WHERE extname = 'pg_cron'
  ) THEN
    -- Remove stale jobs before re-creating (idempotent)
  ...
  ELSE
    RAISE NOTICE 'pg_cron not enabled — schedule workers via Edge Functions or external cron';
  END IF;
END;
$$;
